module CorreiosETC
  module Methods
    class CalculaPrecoPrazo
      ...
      def url
        uri = URI "http://ws.correios.com.br/calculador/CalcPrecoPrazo.asmx/CalcPrecoPrazo?nCdEmpresa=#{CorreiosETC.administrative_code}&sDsSenha=#{CorreiosETC.password}&nCdServico=#{object.service}&sCepOrigem=#{object.cep_sender}&sCepDestino=#{object.cep_receiver}&nVlPeso=#{object.weight}&nCdFormato=#{object.type_object}&nVlComprimento=#{object.width}&nVlAltura=#{object.height}&nVlLargura=#{object.length}&nVlDiametro=#{object.diameter}&sCdMaoPropria=#{object.own_hand}&nVlValorDeclarado=#{object.value}&sCdAvisoRecebimento=#{object.receiving_notice}"
        return uri
      end
